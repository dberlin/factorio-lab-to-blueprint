...
def _rectangle_audit_skeleton() -> tuple[PlacedBuilding, ...]:
    sorter_id = min(catalog.SORTER_IDS)
    model = catalog.building(sorter_id).model_index
    return (
        PlacedBuilding(item_id=sorter_id, model_index=model, x=0, y=1),
        PlacedBuilding(item_id=sorter_id, model_index=model, x=4, y=1),
    )


def test_rectangle_audit_keeps_a_candidate_with_one_feasible_slack_rectangle() -> None:
    net = _prepared_bound_net(0, (0, 1), (4, 1))
    wall = tuple(
        ((2, y, level), 0)
        for y in (0, 1)
    ...
    audited = freeform_module._incumbent_rectangle_feasibility_audit(
        _prepared_bound_problem(
            buildings=_rectangle_audit_skeleton(),
            nets=(net,),
            blocked=wall,
            route_bounds=(0, 0, 4, 2),
            limit=(0, 0, 4, 2),
    ...
    assert audited.eligible
    assert audited.feasible == 1
    assert not audited.proved_dominated


def test_rectangle_audit_proves_all_incumbent_improving_rectangles_infeasible() -> None:
    net = _prepared_bound_net(0, (0, 1), (4, 1))
    wall = tuple(
        ((2, y, level), 0)
        for y in range(3)
    ...
    assert audited.eligible
    assert audited.tested > 0
    assert audited.feasible == 0
    assert audited.proved_dominated
