...
from flab2bp.layout import routing_domain as rd, validate
...
def test_complete_power_plan_revisits_site_after_required_prefix_changes(
    ...
    spec = BuildSpec(
        groups=(MachineGroup(
            recipe_id="iron-ingot", machine_item_id="arc-smelter", count=1,
        ...
        belt_item_id="conveyor-belt-2", belt_items_per_second=Fraction(12),
        ...
        sorter_pick_stacks=(1, 1, 1, 4), sorter_place_stacks=(1, 1, 1, 4),
        machine_rank="exact", power_tower_item_id="tesla-tower", piler_unlocked=True,
    )
    rules = catalog.BeltAltitudeRules(
        max_z=catalog.belt_max_z(catalog.DEFAULT_LAB_LEVEL),
        vertical_construction=True, storage_level=catalog.DEFAULT_STORAGE_LEVEL,
        lab_level=catalog.DEFAULT_LAB_LEVEL, from_url=False,
    )
    original = rd._power_plan
    trials = []

    def plan(canvas, demand, **kwargs):
        canvas.limit = (6, 9, 26, 15)
        allowed = {(17, 10), (26, 15)}
        if available_sites == 3:
            allowed.add((6, 9))
        canvas.keep_out.update(
            (x, y) for x in range(6, 27) for y in range(9, 16)
            if (x, y) not in allowed
        )
        admit = kwargs["complete_plan_failure"]
        ...
        kwargs["complete_plan_failure"] = checked
        return original(canvas, (10, 10, 25, 15), **kwargs)

    monkeypatch.setattr(rd, "_power_plan", plan)
    placement = TransportRoutingKernel(belt_rules=rules, band_policy=BandPolicy("portable")).lay_out(spec)
