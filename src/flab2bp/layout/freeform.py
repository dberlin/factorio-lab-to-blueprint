...
class PreparedRoutingLowerBound:
    ...
    rectangle_eligible: bool = False
    rectangle_tested: int = 0
    rectangle_feasible: int = 0
    rectangle_proved_dominated: bool = False
...
@dataclass(frozen=True, slots=True)
class _RectangleFeasibilityAudit:
    """Audit result for incumbent-improving candidate-local rectangles."""

    eligible: bool
    tested: int
    feasible: int
    proved_dominated: bool
    threshold_area: int
    threshold_belts: int
    wall_time_s: float


def _incumbent_rectangle_feasibility_audit(
    problem: _PreparedRoutingProblem,
    incumbent_key: tuple[int, int] | None,
    *,
    baseline_total: int,
    obstacle_total: int,
) -> _RectangleFeasibilityAudit:
    """Enumerate only rectangles that could beat the current exact incumbent."""
    started = time.perf_counter()
    if incumbent_key is None:
        return _RectangleFeasibilityAudit(
            False, 0, 0, False, 0, 0, time.perf_counter() - started
        )
    protected = _protected_template_belt_indices(problem)
    survivors = tuple(
        building
    ...
    right = max(building.x + building.width - 1 for building in survivors)
    top = max(building.y + building.height - 1 for building in survivors)
    skeleton_width = right - left + 1
    skeleton_height = top - bottom + 1
    skeleton_area = skeleton_width * skeleton_height
    if incumbent_key <= (skeleton_area, baseline_total):
        return _RectangleFeasibilityAudit(
            False,
            0,
            0,
    ...
    # Equal-area rectangles remain contenders only when their candidate-local
    # belt floor is lower than the incumbent's belt count.
    max_area = incumbent_key[0] if obstacle_total < incumbent_key[1] else incumbent_key[0] - 1
    if skeleton_area > max_area:
        return _RectangleFeasibilityAudit(
            True,
            0,
            0,
    ...
    route_left, route_bottom, route_right, route_top = problem.route_bounds
    if problem.limit is not None:
        limit_left, limit_bottom, limit_right, limit_top = problem.limit
        route_left = max(route_left, limit_left)
        route_bottom = max(route_bottom, limit_bottom)
        route_right = min(route_right, limit_right)
        route_top = min(route_top, limit_top)
    route_width = route_right - route_left + 1
    route_height = route_top - route_bottom + 1
    tested = 0
    feasible = 0
    for width in range(skeleton_width, route_width + 1):
        max_height = min(route_height, max_area // width)
        for height in range(skeleton_height, max_height + 1):
            left_min = max(route_left, right - width + 1)
            left_max = min(left, route_right - width + 1)
            bottom_min = max(route_bottom, top - height + 1)
            bottom_max = min(bottom, route_top - height + 1)
            for rectangle_left in range(left_min, left_max + 1):
                rectangle_right = rectangle_left + width - 1
                for rectangle_bottom in range(bottom_min, bottom_max + 1):
                    rectangle_top = rectangle_bottom + height - 1
                    tested += 1
                    audit = _obstacle_aware_prepared_routing_floor(
                        replace(
                            problem,
                            route_bounds=(
                                rectangle_left,
