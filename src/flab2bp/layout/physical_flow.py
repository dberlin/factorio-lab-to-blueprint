...
def solve(
    ...
    *,
    ...
    balances = [Fraction(0) for _ in range(model.nodes)]
    edges: list[_Edge] = []
    ...
    for index, arc in enumerate(model.arcs):
        ...
        if arc.capacity > arc.lower:
            ...
            edges.append(_Edge(arc.source, arc.sink, arc.capacity - arc.lower, index))
    ...
    prices = [Fraction(0) for _ in model.resources]
    ...
    for index, resource in enumerate(model.resources):
        if resource.kind not in kinds or (resources is not None and index not in resources):
            continue
        limit = resource.capacity - sum(
            (model.arcs[i].lower for i in resource.arcs if active[i]), Fraction(0)
        )
        if limit < 0:
    ...
    for denominator in (1_000_000, 1_000_000_000):
        values = tuple(Fraction(value).limit_denominator(denominator) for value in floats)
    ...
    resource_cost = [Fraction(0) for _ in edges]
...
def _certify_primal(
    ...
    balance = [Fraction(0) for _ in range(model.nodes)]
    ...
    for index, resource in enumerate(model.resources):
        if (
            index in resources
            and sum((flows[i] for i in resource.arcs), Fraction(0)) > resource.capacity
        ):
            return None
    return tuple(flows)
