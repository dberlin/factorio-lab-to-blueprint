...
| **Sorter-on-sorter collider overlap (`EBuildCondition.Collide`)** | `colliders.sorter_box` / `colliders.sorter_collisions` (the inserter branch of `CheckBuildConditions`, plus the `isInserter` XOR excusal), seated by `slots.seated_sorter` (`RefreshBuildPreview` 2090-2190 for the machine end, 2100-2107 for the belt end it drags after it, on the slot indices `slots.emitted_sorter` derives) | `validate.game.sorter_collide`; `freeform._bridge` refuses a column whose seated box hits a standing sorter, via `slots.sorter_seat_is_clear` | Agree — one box function, one seat, two callers. Verified end to end against the blueprint the user force-built: all 66 ends of the 33 sorters the game created, within 0.0091 of a tile | LOW |
...
|---|---|---|
| `game.inserter_paste` (the `ErrorInserterData` snap ladder, `rules.PASTE_*`) | **None.** `slots.attachment` applies `SLOT_REACH` (0.8) but not the paste ladder's `PASTE_RADIAL` (1.6) / `PASTE_LATERAL_EPS` (0.1) branches | ~~Currently harmless~~ — **the SNAP half of that ladder is now measured in the wild and is not harmless.** The user built a refused paste and blueprinted the result: 32 of the 33 sorters that built had MOVED, by 0.57 to 1.22 units, exactly as `lpos = transformedBy.position` says they would. It did not by itself refuse any of them, but any geometry the search reasons about at a tile centre is geometry the paste does not build. `slots.seated_sorter` now applies the snap, and `game.sorter_collide` is the first check to ask its question about the seated sorter rather than the emitted one. The reach and lateral BRANCHES of the ladder are still unported on the search side |
