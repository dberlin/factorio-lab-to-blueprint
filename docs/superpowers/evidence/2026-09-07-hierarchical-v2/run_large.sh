#!/usr/bin/env bash
# The v2 gate's eight large cells, through the SHIPPED CLI.
#
# Same invocation shape as v1's `run_large.sh` -- one candidate policy per run
# (`--candidate-policy <label>`), `--band portable`, `-o <stem>.blueprint.txt`,
# and NO `--workers` so the strategy sees the `None` the CLI passes by default
# (which, after Task 3's `pipeline.py` fix, is what finally reaches
# `_pool_width`).  Two additions:
#
#   * a `titanium-glass` cell, and a `--budget 15` column for it and for belt3
#     all-products, so the stem carries the budget: `large-<label>-<policy>-b<budget>-r<round>`;
#   * the argv goes through `run_cell.py`, which calls `cli.main(argv)` with
#     exactly these arguments and dumps a JSON sidecar of the stats the CLI
#     itself never prints (blocks, cut lanes, nogood skips, gap, player-fed).
#
...
set -u
cd "$(dirname "$0")/../../../.." || exit 1
DIR=docs/superpowers/evidence/2026-09-07-hierarchical-v2
R="$1"
