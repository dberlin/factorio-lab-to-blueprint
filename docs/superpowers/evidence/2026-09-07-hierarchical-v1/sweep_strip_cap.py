"""Strip-cap sweep for the hierarchical strategy on the large URLs.

Two modes:

``--probe``   partition only.  For each cap, report the seed blocks the
              partition produces (count, machines, recipes, logical strips) and
              how many cuts they imply.  Costs milliseconds, so it is what
              decides whether a cap is worth a 60 s build at all.

(default)     one real ``HierarchicalLayout.lay_out`` per cap, recording the
              verdict, wall, area, and the stats the strategy publishes
              (blocks, resplits, cut lanes, strips_max).

The spec is built exactly the way ``tests/layout/conftest.py``'s
``mall_all_products`` fixture builds its spec -- ``canonicalize_dataset`` /
``parse_url`` / ``canonicalize_request`` / ``_build_candidates_canonical`` /
``belt_rules_for_url`` -- against the URL named on the command line, so the
sweep measures the same ``BuildSpec`` ``pipeline.build`` would hand a placer.
"""
...
def build(spec, vertical: bool, cap: int, budget: float, workers: int) -> dict:
...
def main() -> int:
    ...
    ap.add_argument("--caps", default="4,6,8,12")
    ...
    for cap in caps:
        row = (
            probe(spec, cap)
            if args.probe
            else build(spec, vertical, cap, args.budget, args.workers)
        )
