...
## 3b. Levers found on the mall URL (added 2026-09-05, evidence `2026-09-05-scale-levers/mall-profile/`)

A five-objective mall (33 recipes; 935 / 449 / 830 machines across the three candidates) profiled at 60 s and 100 s on the branch with Tasks 1-6 merged. Nothing laid out at either budget; the profile says why:

7. **Access-corridor matcher solves are uncapped under boundary validation (freeform, 71 of 100 s).** `_match_access_corridors` runs one `maximize` per claim rank and then a lexicographic tie-break `minimize` over every corridor choice. When `validate` is given (the boundary-reachability cut, the normal production path since 789625f), the tie-break solve has no `max_deterministic_time`, runs to the preparation deadline, and an `UNKNOWN` raises `_PreparationDeadline`, discarding the candidate. Decision: cap every solve with `_ACCESS_TIE_DETERMINISTIC_WORK` in both modes, and treat a capped-out tie-break as "keep the rank-optimal assignment" (already FEASIBLE from the rank solves) rather than a deadline; the validate/cut loop then proceeds on that assignment. The rank `maximize` solves keep `max_time_in_seconds = remaining` (they decide feasibility) but gain the same deterministic cap; a capped rank solve that is FEASIBLE but not OPTIMAL keeps its objective bound as-is (fewer claims served is a refusal the reservation already reports as `missing`).
8. **One grid per corridor probe (freeform, 8 s).** `_reserve_port_access`'s reachability probes and `assignment_boundary_cut` call `_astar` without a grid, so each search rebuilds one (872 builds, 3.9 s). Decision: build one grid for `bounds` per `_reserve_port_access` call and pass it; `_astar` already falls back to a private grid when the shared one does not cover a start or goal, so this is exact.
9. **`_selected_strips` per anneal state (sequence-pair, 31 of 93 s at 449 machines).** Each state rebuilds all strips with `replace` and recomputes `_staged_static_clearance_keys` for every coater strip. Decision: memoize the `replace`d strip per `(template strip, instance_id, variant index)` and `_staged_static_clearance_keys` per strip geometry key; the coater `west_channel` lift still consults `_staged_static_preclearance_proved` per call, since proofs accumulate during the run.
10. **Harness `--url`.** `prof_harness.py` takes an arbitrary FactorioLab URL so the mall profile is reproducible.

Recorded as follow-ups (§5): sequence-pair prepare at scale (`_projected_coater_junction_bans_by_frame`, `_tap_source`, `_place_coaters`, static-risk projection); freeform `_place_coaters` (`_CleanupSurvivorGraph` peel, `_coater_keepout_hits`); the 935-machine freeform refusal should name the band ceiling.
...
- Mall-profile follow-ups (§3b): sequence-pair prepare at scale (`_projected_coater_junction_bans_by_frame` 6.8 s / 2 calls, `_tap_source` 4.5 s / 498 calls, `_place_coaters` 4.3 s, `_staged_static_relation_projection_risks` 4.0 s / 585 calls at 449 machines); freeform `_place_coaters` (`_CleanupSurvivorGraph.extended_snapshot` peel 4.3 s / 371 calls with 2M `_poll`, `_coater_keepout_hits` 3.2 s / 365 calls); `_power_plan`'s own fill and keep-out stamping (1.5 s per attempt after Task 6); the 935-machine freeform refusal should name the band ceiling.
