...
- **A tap on a pass-through lane is a Pile Sorter** (spec §5.3a, user ruling
  2026-09-06). A pass-through lane is a consumer lane whose item is also in
  `spec.surplus_outputs`, so its tail leaves the block and it never backs up.
  The rate table in `catalog.SORTER_RATE_AT_1` assumes a backed-up belt and
  is silent about this; Task 8b owns the rule. No per-recipe case: the
  hydrogen loop is one instance of the shape.
...
**Interfaces:**
- Consumes: `freeform._pick_sorter(rate, span, machines, tiers, *, stacks, min_place_stack, min_pick_stack)` (line 5979), `freeform._sorter_tiers_for(spec)` (line 5932), the consumer-side call sites at `freeform.py:7179` and `freeform.py:17971`, `validate._sorter_items(ctx)`, `validate.Kind.SORTER`, `validate.Context.spec.surplus_outputs`, `catalog.SORTER_RATE_AT_1` (Pile Sorter is id `2014`, item `sorter-4`).
- Produces:
  - `freeform._pass_through_items(spec: BuildSpec) -> frozenset[str]`
  - `_Canvas.pass_through_items: frozenset[str]` set once where the canvas is built from the spec
  - `@check("flow.pass_through_pile_fed", needs_spec=True, needs_groups=True)`

**Why.** Observed in game on `AMM_URL`: the hydrogen loop closes, but each
refinery's hydrogen tap grabs only a few of the items going past and the
refineries starve. `catalog.SORTER_RATE_AT_1` is the swing rate with cargo
always waiting in the pickup cell, i.e. a belt backed up against its last
consumer. A lane whose tail carries surplus off the block never backs up, so a
one-cargo-per-swing sorter misses every item that passes mid-swing. Only the
Pile Sorter grabs several cargos per swing. The lane shape is
`strip_variants.py:1316-1319`'s: any surplus item with internal destinations.

- [ ] **Step 1: Write the failing tests**

Add to `tests/layout/test_validate.py`, reusing Task 8's
`_self_loop_placement` / `_self_loop_spec` helpers with a new keyword
`tap_tier` (the catalog item id emitted for the four hydrogen input sorters,
default `2014`):

```python
def test_pass_through_tap_below_pile_sorter_is_an_error() -> None:
    """A Mk.III on a lane that never backs up still starves the machine.

    Reported on the energetic-graphite URL: the loop closed, the rate table
    was satisfied, and the refineries starved because the taps caught only
    the hydrogen that happened to pass while they were ready.
    """
    placement = _self_loop_placement(tap_tier=2013)
    report = validate.validate(placement, _self_loop_spec(), expect_power=False)
    findings = [f for f in report.errors if f.check == "flow.pass_through_pile_fed"]
    assert len(findings) == 4
    assert findings[0].detail["item"] == "hydrogen"
    assert "Pile Sorter" in findings[0].message


def test_pass_through_tap_with_pile_sorter_passes() -> None:
    placement = _self_loop_placement(tap_tier=2014)
    report = validate.validate(placement, _self_loop_spec(), expect_power=False)
    assert not [f for f in report.findings if f.check == "flow.pass_through_pile_fed"]


def test_dead_end_lane_tap_is_not_a_pass_through_tap() -> None:
    """A lane that ends at its last consumer backs up; Mk.I is fine there."""
    placement = _self_loop_placement(tap_tier=2011)
    spec = _self_loop_spec().model_copy(update={"surplus_outputs": {}})
    report = validate.validate(placement, spec, expect_power=False)
    assert not [f for f in report.findings if f.check == "flow.pass_through_pile_fed"]
```
...
```python
def test_pass_through_items_are_surplus_items_with_internal_consumers() -> None:
    spec = _self_loop_spec()  # hydrogen: consumed by x-ray-cracking, in surplus_outputs
    assert freeform._pass_through_items(spec) == frozenset({"hydrogen"})
    plain = spec.model_copy(update={"surplus_outputs": {}})
    assert freeform._pass_through_items(plain) == frozenset()


def test_pass_through_tap_is_picked_as_a_pile_sorter() -> None:
    """The picker's cheapest-first walk is overridden on a pass-through lane."""
    spec = _self_loop_spec()
    tiers = freeform._sorter_tiers_for(spec)
    tier, _ = freeform._pick_sorter(
        Fraction(1, 2), 2, 1, freeform._tap_tiers(tiers, "hydrogen", freeform._pass_through_items(spec))
    )
    assert tier == 2014
    tier, _ = freeform._pick_sorter(
        Fraction(1, 2), 2, 1, freeform._tap_tiers(tiers, "refined-oil", freeform._pass_through_items(spec))
    )
    assert tier == 2011
```
...
```bash
uv run pytest tests/layout/test_validate.py -k pass_through -x -q
uv run pytest tests/layout/test_freeform.py -k pass_through -x -q
```

Expected: all FAIL (no such check, no `_pass_through_items`, no `_tap_tiers`).

- [ ] **Step 3: Implement the planner side**

In `src/flab2bp/layout/freeform.py`, beside `_sorter_tiers_for` (line 5932):

```python
def _pass_through_items(spec: BuildSpec) -> frozenset[str]:
    """Items whose consumer lane also carries surplus off the block.

    Such a lane never backs up against its last consumer, so a tap on it must
    be a Pile Sorter: ``catalog.SORTER_RATE_AT_1`` is the swing rate with cargo
    always waiting in the pickup cell, and on a moving belt a one-cargo sorter
    misses every item that passes while it is mid-swing.  Observed in game on
    the energetic-graphite URL, where the hydrogen loop closed and the
    refineries starved anyway.  The shape is the lane's, not the recipe's:
    ``strip_variants`` builds it for any surplus item with internal
    destinations, and a self-loop item is one such item.
    """
...
def _tap_tiers(
    tiers: tuple[int, ...], item: str, pass_through: frozenset[str]
) -> tuple[int, ...]:
    """The tiers a tap may be picked from: only the Pile Sorter on a pass-through lane.

    When the save has no Pile Sorter the full list is returned unchanged, so
    ``_pick_sorter``'s fastest-allowed fallback emits what the save can build
    and ``flow.pass_through_pile_fed`` refuses the placement naming the
    research -- the same division of labour as ``flow.sorter_capacity``.
    """
    if item not in pass_through:
        return tiers
    piled = tuple(t for t in tiers if t == _PILE_SORTER_ID)
    return piled or tiers
```

with `_PILE_SORTER_ID = 2014` defined beside it and a comment pointing at
`catalog.SORTER_RATE_AT_1`. Add `pass_through_items: frozenset[str]` to
`_Canvas`, populated from `_pass_through_items(spec)` at the one place the
canvas is built from the spec (find it with Serena: the constructor call that
also sets `lane_stacks`). At the two consumer-side call sites,
`freeform.py:7179` (`into_machine` branch) and `freeform.py:17971`, pass
`_tap_tiers(tiers, item, canvas.pass_through_items)` where `tiers` is passed
today. Do not touch the producer-side call sites (`2356`, `6827`).

- [ ] **Step 4: Implement the check**

In `src/flab2bp/layout/validate.py`, directly after `_sorter_capacity`
(line 5611):
...
def _pass_through_pile_fed(ctx: Context) -> Iterable[Finding]:
    ...
    ``flow.sorter_capacity`` cannot see this defect: its rate table assumes a
    belt backed up against the tap, and a lane that also carries surplus off
    the block is never backed up.  A lower tier on such a lane validates and
    starves the machine in game (energetic-graphite URL, 2026-09-06).
    """
    assert ctx.spec is not None
    consumed = {item for g in ctx.spec.groups for item in g.inputs_per_machine}
    pass_through = {item for item in ctx.spec.surplus_outputs if item in consumed}
    if not pass_through:
        return
    items = _sorter_items(ctx)
    for i, s in ctx.of_kind(Kind.SORTER):
        item = items.get(i)
        if item not in pass_through or not _picks_from_belt_into_machine(ctx, i, s):
            continue
        if s.item_id == cat.PILE_SORTER_ID:
            continue
        yield Finding(
            "flow.pass_through_pile_fed",
            Severity.ERROR,
            f"sorter {i} picks {item} from a pass-through lane with tier {s.item_id}; "
            "that lane never backs up, so only a Pile Sorter (research "
            "'Pile Sorter') grabs its full share",
            (i,) + tuple(x for x in (s.input_obj, s.output_obj) if x is not None),
            {"sorter": i, "item": item, "tier": s.item_id},
        )
```

`cat.PILE_SORTER_ID = 2014` goes into `src/flab2bp/dsp/catalog.py` beside
`SORTER_RATE_AT_1` (and `freeform._PILE_SORTER_ID` reads it rather than
repeating the literal). `_picks_from_belt_into_machine(ctx, i, s)` is the
predicate `flow.sorter_capacity`'s `_sorter_demand` already uses to decide a
sorter is an input tap (input side a belt, output side a machine); if it is
inline there, extract it so both checks share one definition rather than two
copies of the same test.

- [ ] **Step 5: Run and see them pass, then the two suites**

```bash
uv run pytest tests/layout/test_validate.py -k pass_through -x -q
uv run pytest tests/layout/test_freeform.py -k pass_through -x -q
uv run pytest tests/layout -q -x -p no:cacheprovider
```

Expected: exit 0. If any existing fixture with a surplus item and internal
consumers now fails on `flow.pass_through_pile_fed`, that fixture emits the
in-game starvation this task removes: change the fixture's expected tier to
`2014`, and say so in the commit message with the fixture's name.

- [ ] **Step 6: Commit**

```bash
git add src/flab2bp/dsp/catalog.py src/flab2bp/layout/freeform.py \
  src/flab2bp/layout/validate.py tests/layout/test_validate.py tests/layout/test_freeform.py
git commit -m "fix(layout): pile sorters on every pass-through tap

A lane that also carries surplus off the block never backs up, so a
one-cargo sorter on it grabs only the items that pass while it is ready.
Observed in game on the energetic-graphite URL. The picker restricts such
taps to the Pile Sorter and flow.pass_through_pile_fed convicts anything
else."
```
...
5. `decode-amm-after.txt` shows every x-ray-cracking refinery's hydrogen
   input sorter as item id `2014` (Pile Sorter). If `probe_decode.py` does
   not print the sorter's item id on its `from run` lines, add it there first
   and re-run the probe on `probes/bp-seqpair-outputproducts.txt` so the
   before/after pair is in the same format (before: `2011`).
