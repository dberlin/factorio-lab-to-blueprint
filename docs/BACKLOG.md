...
4. **Sorters likewise** -- ~~still not done~~ **DONE**, and the reason it was
   open was right: the box has to be REBUILT, and the ends have to be SEATED.
   Both are now ported and both were needed. `game.sorter_collide` is a
   default-ERROR check.

   The rule, from the decompiled `Assembly-CSharp.dll` rather than from
   inference. `BuildTool_BlueprintPaste.CheckBuildConditions` does not test a
   sorter's prefab box where the record puts it. It stretches it:

   ```
   colliderData.ext = new Vector3(ext.x, ext.y,
       Vector3.Distance(lpos2, lpos) * 0.5f + ext.z - 0.5f);
   if (ObjectIsBelt(inputObjId) || (input != null && input.desc.isBelt))
   { colliderData.pos.z -= 0.35f; colliderData.ext.z += 0.35f; }
   ... and the mirror image for the output end ...
   if (colliderData.ext.z < 0.1f) colliderData.ext.z = 0.1f;
   colliderData.pos = vector2 + quaternion * colliderData.pos;
   ```

   with `vector2 = Vector3.Lerp(lpos, lpos2, 0.5f)` and `quaternion =
   Quaternion.LookRotation(lpos2 - lpos, ...)`. So the box is centred on the
   MIDPOINT, runs the length of the sorter, and grows 0.35 past any end that
   meets a belt or meets nothing -- which is why a belt-to-belt sorter is so
   much wider than its span. `BuildPreviewModel.SetCollider` repeats the same
   stretch for the box a query HITS, on a transform `BuildModel.AddPreviewModel`
   seats at the same midpoint, so both sides of the test are this box; and
   `BuildTool_BlueprintPaste.ActiveColliders` is what puts those colliders in
   the world during a paste, so it is the paste's test and not only the
   interactive tool's. The excusal at `CheckBuildConditions` is an exclusive OR
   on `isInserter`, so sorter-against-sorter is the ONE pairing it does not
   forgive, and the second pass over `Collide` sorters re-runs the identical
   query.
   ...
   **The answer key, from the game.** The user force-built a refused paste and
   ...
   machines, and 33 of 38 sorters; it dropped five. `game.sorter_collide` names
   those five and no others. On the 1132 sorters of the single-area fixture
   corpus it is silent, and that sample is not vacuous -- the same corpus
   overlaps sorter BODIES on a shared tile 53 times and carries belt-to-belt
   sorters, both legal, both untouched by this predicate. Both blueprints are
   committed under `tests/fixtures/ours/`.

   ~~One measurement worth having before anyone starts: across 16 corpus cells
   both strategies emit 702 sorters with 172 anchor points shared by two or more
   of them, against zero in the game's 1132.~~ **That divergence was the
   symptom, and it is a symptom rather than the rule: a shared anchor is neither
...
## RESOLVED -- layouts are certified for an explicit latitude-band policy

The open handoff below is complete. `dsp/planet.py` is the shared source for the game's
longitude quantisation; both production strategies feed one `BandPolicy` into projection-aware
search and authoritative finalization; and the encoder reads the finalized immutable
`AreaFrame` rather than a parallel stats field.

The public selection is exactly:

```text
portable | 4 | 8 | 16 | 20 | 32 | 40 | 60 | 80 | 100 | 120 | 160 | 200
```

`portable` is the CLI, pipeline, and web default. It fixes `B0` as the globally smallest band
in which the unpadded content fits, then certifies `B0` and up to two bands with greater
`area_segments`. It does not move to a wider primary band to evade a failure, and it does not
claim every band on the planet. For example, a primary band of 160 certifies `(160, 200)`;
primary 200 certifies `(200,)`.

For each required band, the identical orientation, buildings, and frame are checked at every
legal latitude anchor. Finalization searches zero through four added latitude rows, every
north/south split that still fits `B0`, in deterministic minimum-area order. There is no
longitude padding: shifting longitude rotates the frame around the planet without changing its
relative projected geometry.

An explicit numeric selection certifies that requested band and no other band, with the same
up-to-four-row latitude-padding search. A frame that does not fit the requested band or fails
at any legal anchor refuses; there is no fallback to Portable or a different numeric band.
...
positional addon supply area now counts as a termination tap. The existing
`flow.external_entry_points` behavior is unchanged: multiple reachable external lanes for one
item remain legal, warn that the player must connect every lane, and do not block emission.

The final Task 9 comparison is `/tmp/portable-band-comparison.json`, SHA-256
`d2f2f2732f6bf67bc1b5e2051fb081c4b36ea8a48d57a9e6c0b89de4d808bd9f`. Its strict projection
threshold passed 3/3 cases: added p95 was 0.814080129s (small), 0.135248086s (medium), and
0.925458988s (large), each below the approved 1.0s ceiling. The semantic-aware loaded-machine
build gate passed 60/60 using the requested 4.0s budget plus the approved 5.0s atomic-completion
grace (9.0s total).

That result is not a claim that the original unconditional 10% wall-time comparison passed.
Forty-three of 60 cells remain recorded as historical greater-than-10% diagnostics; the
semantic-aware gate distinguishes work that now performs the required multi-band
certification and can legitimately refuse from a like-for-like timing regression.
